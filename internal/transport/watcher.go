...
// exemptSessions lists session directories that hold autonomous state files
// rather than interagent protocol messages. Per transport.md §Exempt Session
// Types: these sessions use turn=0 as a sentinel, do not follow monotonic
// turn numbering, and MUST NOT trigger transport event processing.
var exemptSessions = map[string]bool{
	"local-coordination": true,
}

// exemptSession returns true if the session directory name matches an exempt
// session type. Exempt sessions contain heartbeat, mesh-state, halt, and
// escalation files generated by meshd — not inter-agent messages.
func exemptSession(name string) bool {
	return exemptSessions[name]
}
